...
import lk.ijse.dep.pos.JPAConfig;
...
public class MainFormController implements Initializable {
    ...
    private String getUsername(){
        return DEPCrypt.decode(environment.getRequiredProperty("javax.persistence.jdbc.user"),"dep4");
    }
    private String getPassword(){
        return DEPCrypt.decode(environment.getRequiredProperty("javax.persistence.jdbc.password"),"dep4");
    }

    private String getHost(){
        return environment.getRequiredProperty("ijse.dep.ip");
    }

    private String getPort(){
        return environment.getRequiredProperty("ijse.dep.port");
    }

    private String getDatabase(){
        return environment.getRequiredProperty("ijse.dep.db");
    }


    public void initialize(URL url, ResourceBundle rb) {
        environment = AppInitializer.ctx.getBean(Environment.class);
    ...
    public void btnRestore_OnAction(ActionEvent actionEvent) {
        ...
        if (file != null) {
            ...
            if(getPassword().length()>0) {
                commands = new String[]{"mysql", "-h", getHost(),"--port", getPort(),"-u", getUsername(), "-p" + getPassword(), getDatabase(),
                        "-e", "source " + file.getAbsolutePath()}; // need a space after "source "}
            }else{
                commands = new String[]{"mysql", "-h", getHost(),"--port", getPort(), "-u", getUsername(), getDatabase(),
                        "-e", "source " + file.getAbsolutePath()}; // need a space after "source "}
    ...
    public void btnBackup_OnAction(ActionEvent actionEvent) {
        ...
        if (file != null) {
            Task task =  new Task<Void>(){
                ...
                @Override
                protected Void call() throws Exception {
                    Process process = Runtime.getRuntime().exec("mysqldump -h" + getHost() +" --port "+getPort()+ " -u" + getUsername() + " -p" + getPassword() + " " + getDatabase() + " --result-file " + file.getAbsolutePath() + ((file.getAbsolutePath().endsWith(".sql")) ? "" : ".sql"));
                    int exitCode = process.waitFor();
